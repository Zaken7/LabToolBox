...
master2=192.168.145.203
master3=192.168.145.204
worker1=192.168.145.205
worker2=192.168.145.206
worker3=192.168.145.207
...
lbrange=192.168.145.220-192.168.145.235
...
#echo "give permissions to root for: $(certName)"
...
# Clean all SSH host keys before copying new ones
echo -e " \033[33;5mCleaning existing SSH host keys...\033[0m"
for node in "${all[@]}"; do
    echo "Removing host key for: $node"
    ssh-keygen -f "/home/ubuntu/.ssh/known_hosts" -R "$node" 2>/dev/null || true
done
echo -e " \033[32;5mAll SSH host keys cleaned!\033[0m"
...
for newnode in "${all[@]}"; do
    ssh $user@$newnode -i ~/.ssh/$certName sudo NEEDRESTART_MODE=a apt-get install policycoreutils -y
